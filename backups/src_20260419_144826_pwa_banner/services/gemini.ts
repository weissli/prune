...
export const identifyPlant = async (imageBase64: string, apiKey: string) => {
  if (!apiKey) throw new Error("API Key required");
  ...
  const prompt = `
    ...
    Return a JSON object with the following structure:
    {
      "matches": [
        {
          "name": "Plant Name",
          "description": "Short description of the plant (1-2 sentences max)",
          "confidence": "high" | "medium" | "low"
        }
      ]
    }
    Provide at least 3 and up to 5 possible matches if there is any ambiguity.
    Even if you are confident, provide alternative possibilities or related species to give the user options.
    Keep the descriptions short (1-2 sentences) to fit well in a list UI.
  `;
