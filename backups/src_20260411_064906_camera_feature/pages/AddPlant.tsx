  ...
  const handleCapture = async () => {
    ...
    try {
      ...
      if (results.matches && results.matches.length > 0) {
        ...
        if (bestMatch.confidence === 'high' || results.matches.length === 1) {
          setName(bestMatch.name);
          handleGenerate(bestMatch.name);
        } else {
      ...
      <div className="p-6 max-w-2xl mx-auto pb-32">
        ...
        <div className="space-y-6">
          {searchResults.map((result, index) => (
            <div key={index} className="bg-white p-4 rounded-2xl border border-slate-200 flex gap-4 items-center">
              <div className="w-20 h-20 rounded-xl overflow-hidden bg-slate-100 shrink-0">
                <img 
                  src={`https://source.unsplash.com/featured/?${encodeURIComponent(result.name)},plant`} 
                  alt={result.name} 
