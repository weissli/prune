...
import { ArrowLeft, Camera, Loader2, Sparkles, Calendar } from 'lucide-react';
...
export const AddPlant = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const success = addPlant({
      ...
      pruningTasks: pruningTasks || (selectedMonths.length > 0 ? [{ name: 'Pruning', type: 'recurring', months: selectedMonths }] : []),
    ...
    return (
      <div className="bg-white min-h-screen pb-24">
        ...
        <div className="relative h-72 w-full bg-slate-100">
          <img 
            src={imageUrl || `https://loremflickr.com/600/400/${encodeURIComponent(name)},plant`} 
            alt={name} 
            className="w-full h-full object-cover"
            onError={(e) => {
              (e.target as HTMLImageElement).src = "https://images.unsplash.com/photo-1542601906990-b4d3fb778b09?w=600&h=400&fit=crop";
        ...
        <form onSubmit={handleSubmit} className="p-6 -mt-6 bg-white rounded-t-3xl relative z-10 space-y-6 max-w-2xl mx-auto">
          <div>
          ...
          <div className="space-y-4">
            ...
            {pruningTasks && pruningTasks.length > 0 ? (
                  ...
                  <div key={task.name} className="p-4 bg-slate-50 rounded-2xl border border-slate-100 space-y-2">
                    ...
                    <div className="flex flex-wrap gap-2">
                      {task.months.map(m => (
                        <span
                          key={m}
                          className="px-3 py-1.5 bg-white text-slate-700 border border-slate-200 rounded-full text-xs font-medium"
                        >
                          {MONTH_NAMES[m]}
                        </span>
                      ))}
      ...
      <form onSubmit={handleSubmit} className="space-y-8">
        ...
        <div className="space-y-2">
          ...
          <div className="flex gap-2">
            ...
            >
              {isIdentifying ? <Loader2 className="animate-spin" /> : <Camera className="text-slate-600" />}
            </Button>
            <Button 
              type="button" 
              variant="secondary"
